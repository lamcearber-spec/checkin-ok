...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <p className="text-gray-400 mb-8">{t('welcomeBack')}{user?.name ? `, ${user.name}` : ''}.</p>
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        <div className="bg-[#112240] rounded-xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4"><div className="p-2 bg-green-500/10 rounded-lg"><BarChart3 className="text-green-500" size={20} /></div><h3 className="text-gray-400 text-sm">{t('plan')}</h3></div>
          <p className="text-2xl font-bold text-white">{tierLabel[user?.tier || 'FREE'] || user?.tier}</p>
        </div>
        <div className="bg-[#112240] rounded-xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4"><div className="p-2 bg-emerald-500/10 rounded-lg"><FileText className="text-emerald-400" size={20} /></div><h3 className="text-gray-400 text-sm">{t('usage')}</h3></div>
        ...
        <div className="bg-[#112240] rounded-xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4"><div className="p-2 bg-purple-500/10 rounded-lg"><Upload className="text-purple-400" size={20} /></div><h3 className="text-gray-400 text-sm">{t('quickActions')}</h3></div>
          <div className="space-y-3">
            ...
            <Link href="/history" className="flex items-center justify-between text-white hover:text-green-400 transition-colors text-sm">{t('viewHistory')} <ArrowRight size={16} /></Link>
            <Link href="/pricing" className="flex items-center justify-between text-white hover:text-green-400 transition-colors text-sm">{t('upgradePlan')} <ArrowRight size={16} /></Link>
          </div>
        </div>
      </div>
    </div>
